...
namespace Lifti.Serialization.Binary
{
    internal class V1IndexReader<TKey> : IIndexReader<TKey>
    {
        ...
        private IndexNode DeserializeNode(IIndexNodeFactory nodeFactory, int depth)
        {
            ...
            var childNodes = childNodeCount > 0 ? new Dictionary<char, IndexNode>() : null;
            var matches = matchCount > 0 ? new Dictionary<int, ImmutableList<IndexedWord>>() : null;
            ...
            return nodeFactory.CreateNode(intraNodeText,childNodes, matches);
        }
